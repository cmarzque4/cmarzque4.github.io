const questions = [
    ...
    },
    {
        ...
        roda del darrere. La marxa que té posada fa que la relació de transmissió entre l’eix de sortida del motor i l’eix de la roda del darrere sigui \\(τ = ω_{\\text{roda}}/ω_{\\text{motor}} = 0{,}044\\). Aquesta reducció de
        velocitat s’aconsegueix mitjançant una transmissió d’engranatges de rendiment \\(η_{\\text{eng}} = 0{,}90\\) i
        una transmissió per cadena de rendiment \\(η_{\\text{cad}} = 0,85\\). La roda té un diàmetre \\(d = 620\\,\\text{mm}\\) i la
        massa total de la motorista més la moto és \\(m = 150\\,\\text{kg}\\). Si el parell a l’eix de sortida del motor
        és \\(Γ_{\\text{motor}} = 6\\,\\text{N m}\\) i la fricció amb l’aire es considera negligible, determineu: 
        <img src="ex.moto.png" alt="Imatge moto">
            <br><br>
            <strong>a)</strong> Les velocitats de gir de la roda \\(ω_{\\text{roda}}\\) i de l’eix del motor \\(ω_{\\text{motor}}\\), en rad/s. [0,5 punts]
            <br>
            <strong>b)</strong> La potència \\(P_{\\text{motor}}\\) a l’eix de sortida del motor. [0,5 punts]
            <br>
            <strong>c)</strong> La potència mecànica \\(P_{\\text{roda}}\\) a l’eix de la roda i l’angle α que forma el perfil de la carretera amb l’horitzontal. [1 punt]
            <br>
            <br><strong>d)</strong> El parell \\(Γ_{\\text{roda}}\\) a l’eix de la roda. [0,5 punts]
            <br><br>`,
        correctAnswer: "",  
        steps: `
            <strong>a) Velocitat angular de les rodes i del motor:</strong>
    ...
    },
    {
        ...
        \\(d = 2{,}5\\,\\text{mm}\\). El coure té una resistivitat \\(ρ = 1{,}7 x 10{<sup>–8</sup>}\\,\text{Ω m}\\). La resistència \\(R_{\\text{cable}}\\) de l’esquema de
        la figura correspon als dos conductors del cable bipolar. Si cada llum té una potència nominal
        \\(P_{\\text{llum}} = 55\\,\\text{W}\\) quan s’alimenta directament amb una tensió \\(U = 12\\,\\text{V}\\), determineu:
         
            <br><br>
            <strong>a)</strong> La resistència equivalent R_{\\text{eq}}\\) dels dos llums connectats en paraŀlel. [0,5 punts]
            <br>
            <strong>b)</strong> La longitud màxima Lmàx que pot tenir el cable si es vol que la caiguda de tensió del cable no sigui superior al 5 %. [1 punt]
            <br><br>
           Si s’utilitza un cable amb una longitud \\(L = 4\\,\\text{m}\\), determineu:
           <br>
            <strong>c)</strong>La resistència del cable Rcable. [1 punt]
            <br>
            <br><strong>d)</strong> La potència total Ptotal consumida conjuntament pel cable i els dos llums. [0,5 punts]
            <br><br>`,
        correctAnswer: "",  
        steps: `
            <strong>a) Velocitat angular de les rodes i del motor:</strong>
